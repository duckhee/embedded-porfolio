...
//     User_notification(&gnetif);
    ...
    /** CAN에 대한 설정을 변경을 하는 함수 호출 */
    HAL_CAN_ConfigFilter(&hcan1, &canFilter1);
    /** CAN에 대한 데이터가 수신이 되었을 때 Interrupt가 발생하도록 설정 */
    ...
    while (1) {
        ...
        if (can1_Rx0_Flag) {
            can1_Rx0_Flag = 0;
            CLCD_Clear();
            sprintf(str, "Rx ID : 0x%03X", canRxHeader.StdId);
            CLCD_Puts(0, 0, str);

            sprintf(str, "Rx Data : 0x%02X", can1Rx0Data[0]);
            CLCD_Puts(0, 1, str);
        }
        if (Sw1_Flag) {
            Sw1_Flag = 0;
...
/** CAN에 대한 데이터 수신 시 발생할 Interrupt Handler */
