...
//     User_notification(&gnetif);
  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  /** file 내용을 32byte로 읽어서 코덱에 전달 */
	  /** Codec이 준비가 되었는지 확인하기 위한 조건 및 대기 */
	  if(MP3_DREQ == 1){
		  /** 재생 값이 설정이 되어 있을 경우 */
		  if(pp_flag){
			  /** 파일에 대해서 읽어오기 -> br 의 값을 보고 실제로 읽은 파일의 바이트를 확인이 가능하다. */
			  f_read(&SDFile, readBuffer, 32, &br);
			  if(br >= 32){
				  /** Codec 에 전달 */
				  VS1003_WriteData(readBuffer);
			  }
			  /** 32byte 가 아닌 경우 */
			  else{
